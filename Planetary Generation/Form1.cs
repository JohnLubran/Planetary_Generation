...
namespace Planetary_Generation
{
    public partial class Form1 : Form
    {
        ...
        {
            directoryInput.Text = Properties.Settings.Default.Directory;
            sizeInput.Text = "1000";
        ...
        private void GenerationButton_Click(object sender, EventArgs e)
        {
            ...
            PlateLayerI.SavePlates(directoryInput.Text, outNameInput.Text);
            pictureBox.ImageLocation = directoryInput.Text + "\\" + outNameInput.Text + ".png";
            Rectangle resolution = Screen.PrimaryScreen.Bounds;
            //if (resolution.Width > 2 * size && resolution.Height > size - 280)
            //{
            //    pictureBox.Width = 2 * size;
            //    pictureBox.Height = size;
            //    Form.ActiveForm.Width = 2 * size;
            //    Form.ActiveForm.Height = size + 280;
            //}
            //else
            //{
            //    pictureBox.Width = resolution.Width;
            //    pictureBox.Height = resolution.Height - 280;
            //    Form.ActiveForm.Width = resolution.Width;
            //    Form.ActiveForm.Height = resolution.Height;
            //}
        ...
        private void MovePlateButton_Click(object sender, EventArgs e)
        {
            ...
            if (!PlateLayerI.HeightInputs(directoryInput.Text, heightMapInput.Text))
            {
                errorDisplay.Text += "Height Map input is unreadable. ";
                return;
            }
            PlateLayerI.PlatesVelocityInputs(speeds, directions);
            if (!PlateLayerI.PlatesMapInputs(directoryInput.Text, plateMapInput.Text))
            {
                errorDisplay.Text += "Plate Map input is unreadable. ";
                return;
            }
            PlateLayerI.PlateMove(timeStep);
            PlateLayerI.SaveHeights(directoryInput.Text, heightMapInput.Text);
            PlateLayerI.SavePlates(directoryInput.Text, plateMapInput.Text);
